...
task :generate do
  ...
  system "cd public/Blog; ln -f -s ../index.html ."
end
...
desc "serve the site in a web browser"
task :serve do
  raise "### You haven't set anything up yet. First run `rake install` to set up an Octopress theme." unless File.directory?(source_dir)
  puts "Building site with Jekyll and Compass. Starting Rack on port #{server_port}"
  system "compass compile --css-dir #{source_dir}/stylesheets" unless File.exist?("#{source_dir}/stylesheets/screen.css")
  system({"OCTOPRESS_ENV"=>"preview"}, "jekyll build --trace")
  system "cd public/Blog; ln -f -s ../index.html ."
...
task :rsync do
  ...
  ok_failed system("rsync -cavze 'ssh -p #{ssh_port}' #{exclude} #{rsync_args} #{"--delete" unless rsync_delete == false} #{public_dir}/ #{ssh_user}:#{document_root}")
end
