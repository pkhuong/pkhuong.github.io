...
[Safe memory reclamation]() is a key component for lock-free synchronisation when garbage collection isn't an option,[^it-is-gc]
and [hazard pointers]() let us bound resource leaks much more tightly than, e.g., [epoch reclamation]().
...
Blelloch's and Wei's wait-free algorithm eliminates that loop... with a construction that stacks [two emulated primitives---atomic copy, itself implemented with strong LL/SC---](https://arxiv.org/abs/1911.09671)on top of what hardware usually offers.
I see the real value of the construction in showing that wait-freedom is achievable,
and that the key is atomic memory-memory copies.
In this post, I'll show how a bit of engineering work makes that construction practical,
...
in practice, lock-free algorithms only need to pin a few (usually one or two) nodes at a time to ensure memory safety.  As long as we can avoid running arbitrary code while holding hazardous references, we can bound the number of records each thread may need at any one time.
...
we'll instead *guess* what resource we will borrow,
publish that,
...
On an ideal machine with a [sequential consistency memory model](https://en.wikipedia.org/wiki/Sequential_consistency),
...
[^beware-accidental-success]: I tend to implement lock-free algorithms with a heavy dose of inline assembly, or [Concurrency Kit](http://concurrencykit.org/)'s wrappers: it's far too easy to run into subtle undefined behaviour.  For example, comparing a pointer after it has been freed is undefined behaviour in C and C++, even if we don't access the pointee.  Even if we compare as `uintptr_t`, it's apparently debatable whether the code is well defined when the comparison happens to succeed because the pointee was freed, then recycled in an allocation and published to `cell` again.

We need a store/load fence in `R1` to make sure the store to the record is visible before `R2` is visible by the time the second read executes.  Under the [TSO memory model used by x86 chips (PDF)](https://www.cl.cam.ac.uk/~pes20/weakmemory/x86tso-paper.tphols.pdf),
...
so we can emulate the fence in `R1`
...
When we use fences or atomic instructions correctly, there should be
...
emulates an atomic memory-memory copy.  If only we had such an atomic copy primitive, the read-side would be trivial.
...
However, we've already noted that syscalls like `membarrier` mean we can lower our requirements to interrupt atomicity, i.e., any individual instruction works since we're assuming precise interrupts... and it just happens that `x86` and `amd64` do have an instruction for memory-memory copies!
...
We can improve that with a trick from classic hazard pointers, and
...
`gen_sequence`, and write that to `pinned_or_gen`.  We'll tag
...
struct hp_record_wf {
        ...
        struct {
                void **volatile cell;
                volatile intptr_t pinned_or_gen;
        } help;
};
{% endcodeblock %}
...
in `help.cell`.  That's enough to read the cell's value, and update to
the address is just read with a compare-and-swap (CAS) of
`pinned_or_gen`: if the CAS fails, another helper got there first, or
...
that any helper's write to `record.help.pin_or_gen` are visible
before we compare it against `gen`:
...
`pinned_or_gen` before we consumed the list, help these sections
...
That's a couple more instructions on the read-side that the
...
We can still do better, by further specialising the code.  The cleanup
...
We'll also use tagging with negative of positive values, this time to
...
Again, `RA` matches `C1`, and `RB` `C2`.  This new implementation is as fast as `hp_read_wf` on the read side, and needs even fewer instructions.
...
lock-free membarrier implementation.  In either case, we rely on
...
roughly the same, and instruction bytes comparable to `hp_read_membarrier`.
However, we've introduced more opportunities for reclamation hiccups.
...
in general I suppose we can install signal handlers to catch `SIGSEGV`
and `SIGBUS` in dedicated cleanup threads.
...
microbenchmarking a loop that traverses a pseudorandomly shuffled
circular linked list for a fixed number of pointer chasing hops.

The un-protected (baseline) inner loop follows.  Note the NULL
...
I thought it might be interesting to see what happens when we add work
outside that dependency chain, by letting `work()` be more than an
inline no-op function, but the results will show that's not necessary
for the implementations presented in this post.  The call to
`frob_ptr` lets us introduce work in the dependency chain, which more
closely represents realistic use cases.  For example, when using
hazard pointers to protect a binary search tree traversal, we must
perform a small amount of work to determine whether we want to go down
...
the microbenchmark: we avoid discovering that inserting hazard pointer
...
`head = head->next` is replaced with a call to an inline function.
...
The dependency chain is pretty obvious; we can measure the total
latency for 1000 traversals by running the loop 1000 times.  I'm using
a large iteration count to absorb noise from the timing harness
(roughly 50 cycles per call), as well as any boundary effect around
the first and last few loop iterations.

All the cycle measurements here are from my unloaded E5-4617, running
at 2.9 GHz without turboboost.  First, let's see what happens with a
pure traversal, where `work` and `frob_ptr` are inline no-op functions
that simply return their first argument.  This microbenchmark is far
from realistic (if I found an inner loop that was *only* traversing a
linked list, I'd consider a different data structure), but helps
...
it's not clear there's any additional information to be found in the graphs,
compared to the tables below.

The next table shows the cycle counts for following 1000 pointers in a
circular linked list, with various hazard pointer schemes and *no work to find the next node*, on an unloaded E5-4617 @ 2.9 GHz, without turboboost.
...
The table above reports quantiles the total runtime for 1000
traversals, after one million repetitions.
...
We're looking at a baseline of 4 cycles/node traversal (the L1 cache
...
atomic operation, and for `hp_read_movs`, which inserts a known latency bump in the hot path, the other slowdowns mostly reflect contention for
execution resources.  In real life, such contention mostly occurs in
heavily tuned code, and the actual execution units (ports) in high
demand will vary from one inner loop to the other.
...
dependency on the node value and the multiplication to the pointer
...
loops of 1000 traversals, on an unloaded E5-4617 @ 2.9 GHz.
...
two are on par with the un-protected traversal loops (within 1.1 cycle
...
scheduling hardware.  What really matters is that they all show a
...
slow cleanup (reclamation) path.  If we were to look at the
microbenchmarks alone, one would probably simply implement `hp_read_swf`.
...
and the added requirements of `hp_read_swf` on cleanup threads.  In
...
crashing, and also needs mutual exclusion around the scanning of (sets
of) hazard pointer records.  These addtional requirements don't seem
impractical, but I can imagine code bases where they would constitute
hard blockers (e.g., library code, or when protecting arbitrary integer ids).

TL;DR: Use `hp_read_swf` if you know what you're doing or want more wait-free speed holes; otherwise, `hp_read_movs_spec` is an all-around solid wait-free option on `x86` and `amd64`.
