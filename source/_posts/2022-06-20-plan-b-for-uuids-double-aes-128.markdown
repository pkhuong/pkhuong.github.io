...
That being said, design decisions that enable systems to scale to hundreds of nodes have a tendency to also force you to [distribute work over a dozen machines when a laptop might have sufficed](https://www.usenix.org/system/files/conference/hotos15/hotos15-paper-mcsherry.pdf).
...
let competitors estimate the creation rate and number of accounts (or, similarly, daily activity) in your application,
or simply expose an accidentally powerful API that will be difficult to replace.

Rather than trying to pinpoint the exact level of dedication we're trying to foil, from curious power user to nation state actor, let's aim for something that's hopefully as hard to break as our API or HTTP server's HTTPS.
...
This might be disappointing given that [AES128-CTR could achieve more than 1 byte/cycle in 2013](https://2013.diac.cr.yp.to/slides/gueron.pdf#page=20).
...
the data dependency is necessary to prevent superscalar chips from overlapping multiple iterations...
and that's important for the same reason that it takes hours to cross the Atlantic Ocean, even [if one could hop on a Concorde with a hundred other people](https://www.britishairways.com/en-us/information/about-ba/history-and-heritage/celebrating-concorde).
...
[^rotation]: Rotating symmetric keys isn't hard a technical problem, when generating valid UUIDs with a Feistel network: we can use 1-2 bits to identify keys, and eventually reuse key ids.  Rotation however must imply that we will eventually fail to decode (reject) old ids, which may be a bug or a feature, depending on who you ask; it should at least be possible for a service to update old ids to the most recent symmetric key without accessing any information except the symmetric keys themselves.

<small>Thank you Andrew, Barkley, Jacob, Justin, Marius, and Ruchir, for helping me clarify this post, and for reminding me about things like large (range-sharded) distributed databases.</small>
