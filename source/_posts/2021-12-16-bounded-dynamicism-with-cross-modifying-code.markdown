...
It restricts runtime modifications to boolean values (a flag can
only be enabled or disabled), so hopefully isn't party to
any unexpected compounding complexity.
...
The library can be used for feature flags. However, its minimal
...
proved invaluable for crisis management, since we can leave flags
(enabled by default) in well-trodden pieces of code without agonising
over their impact on application performance. These flags can serve as
ad hoc circuit breakers around complete features or specific pieces of
code (e.g., disabling a destructor to convert use-after-frees into 
resource leaks) when new inputs tickle old latent bugs.
...
Runtime efficiency is an essential feature in `dynamic_flag`--enough
to justify mutating machine code while it's executing on other
cores--not only because it unlocks additional use cases, but, more
...
location, even if, e.g., that's in the middle of an inner loop.
...
about all forms of overhead, not just execution time. For
example, there's no need to break one's flow and register flags
separately from their use points. Adding a feature flag must not
become associated with painful memories.
...
code,[^macro-writers-bill-of-rights] while the interface makes it easy
to make chunks of code togglable at runtime without having to worry
...
all contexts, avoids contention in the source code, and guarantees
...
`dynamic_flag` library is much more powerful. Applications can
...
For example, an application could let individual HTTP requests opt into
detailed tracing with a query string parameter `?tracing=1`, and set
`request->tracing_mode = true` in its internal request object when it
accepts such a request. In environments where fewer than one request
in a million enables tracing, we can easily spend more aggregate time
looking at `request->tracing_mode` than in the tracing logic itself.
One could try to reduce the overhead by coalescing the trace code in
fewer conditional blocks, but that tends to put more distance between
the traced logic and the tracing code that's supposed to record what
the former is doing, so the two tend to desynchronize, adding to
...
[counters associated with each instance of a `DF_...` macro](https://github.com/backtrace-labs/dynamic_flag/blob/00381c2cab5c8628e6a7d18730a98f7d7e6712f2/src/dynamic_flag.c#L99-L110), so this scheme even works when
multiple requests with overlapping lifetimes opt into tracing:
tracing-only blocks will enter the "slow" path (check whether
...
and enable it just before spawning worker threads.

When the program only enters slow phases every few minutes, `DF_OPT`
is also a good option. That was the case for a 
...
check for a slow path, and all these checks can be guarded with
`DF_OPT(stm, ...)` conditions. The program only has to enable `stm`
flags around a batch of updates.  Enabling and disabling all these
...
around bugs by forcing a subset of flags to remain enabled or disabled.

It's easy to force a flag in an active state: flag activations
...
manually, once. However, activation counts don't go negative, to make it safe
...
without wedging the system in a weird state, so we can't use deactivation
...
unhooks `DF_*` conditions by regex on their full name.
While a condition has been
...
that should hopefully suffice to avoid sharp edges around instruction
decoding in hardware, despite disregarding
[Intel's recommendations for cross-modifying code in Section 8.1.3 of the SDM (Volume 3A)](https://www.intel.com/content/dam/www/public/us/en/documents/manuals/64-ia-32-architectures-software-developer-vol-3a-part-1-manual.pdf#page=260).[^cpu-millenia] The library
[relaxes and reinstates page protection with `mprotect(2)`](https://man7.org/linux/man-pages/man2/mprotect.2.html)) around
